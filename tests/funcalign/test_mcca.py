...
def test_mcca():
    ...
    model = brainiak.funcalign.mcca.MCCA()
    assert model, "Invalid MCCA instance!"
    ...
    model = brainiak.funcalign.mcca.MCCA(n_iter=5, features=features)
    assert model, "Invalid MCCA instance!"
    ...
    with pytest.raises(ValueError) as excinfo:
        model.fit(X)
    print("Test: running MCCA with 1 subject")
    ...
    assert len(model.w_) == subjects, "Invalid computation of MCCA! (wrong # subjects in W)"
    for subject in range(subjects):
        assert model.w_[subject].shape[0] == voxels, "Invalid computation of MCCA! (wrong # voxels in W)"
        assert model.w_[subject].shape[1] == features, "Invalid computation of MCCA! (wrong # features in W)"
    ...
    assert model.s_.shape[0] == features, "Invalid computation of MCCA! (wrong # features in S)"
    assert model.s_.shape[1] == samples, "Invalid computation of MCCA! (wrong # samples in S)"

    # Check that it does run to compute the shared response after the model computation
    new_s = model.transform(X)

    assert len(new_s) == subjects, "Invalid computation of MCCA! (wrong # subjects after transform)"
    for subject in range(subjects):
        assert new_s[subject].shape[0] == features, "Invalid computation of MCCA! (wrong # features after transform)"
        assert new_s[subject].shape[1] == samples, "Invalid computation of MCCA! (wrong # samples after transform)"
