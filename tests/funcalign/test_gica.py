...
def test_gica():
    ...
    model = brainiak.funcalign.gica.GICA()
    assert model, "Invalid GICA instance!"
    ...
    model = brainiak.funcalign.gica.GICA(n_iter=5, features=features)
    assert model, "Invalid GICA instance!"
    ...
    with pytest.raises(ValueError) as excinfo:
        model.fit(X)
    print("Test: running GICA with 1 subject")
    ...
    assert len(model.w_) == subjects, "Invalid computation of GICA! (wrong # subjects in W)"
    for subject in range(subjects):
        assert model.w_[subject].shape[0] == voxels, "Invalid computation of GICA! (wrong # voxels in W)"
        assert model.w_[subject].shape[1] == features, "Invalid computation of GICA! (wrong # features in W)"
    ...
    assert model.s_.shape[0] == features, "Invalid computation of GICA! (wrong # features in S)"
    assert model.s_.shape[1] == samples, "Invalid computation of GICA! (wrong # samples in S)"

    # Check that it does run to compute the shared response after the model computation
    new_s = model.transform(X)

    assert len(new_s) == subjects, "Invalid computation of GICA! (wrong # subjects after transform)"
    for subject in range(subjects):
        assert new_s[subject].shape[0] == features, "Invalid computation of GICA! (wrong # features after transform)"
        assert new_s[subject].shape[1] == samples, "Invalid computation of GICA! (wrong # samples after transform)"
